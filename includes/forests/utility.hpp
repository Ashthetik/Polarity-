...
namespace ranger {
    void equalSplit(std::vector<uint>& result, uint start, uint end, uint num);

    template<typename T>
    inline void saveVector1D(const std::vector<T>& vec, std::ofstream& file) {
    ...
    template<>
    inline void saveVector1D(const std::vector<bool>& vec, std::ofstream& file) {
    ...
    template<>
    inline void readVector1D(std::vector<bool>& vec, std::ifstream& file) {
        size_t length;
